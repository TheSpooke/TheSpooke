...
class AplicacionBloqueo:
    ...
    # Get the selected time from the dropdown menu
        ...
        # Check if the entered time is greater than 0
        if tiempo_ingresado > 0:
            # Start the block if it's not already started
            if not self.bloqueo_iniciado:
                self.bloqueo_iniciado = True
                self.tiempo_total = tiempo_ingresado
                self.boton_agregar_mas_tiempo.config(state=tk.NORMAL)  # Enable the button to add more time
    ...
    def bloquear_pantalla(self):
        ...
        self.ventana_bloqueo.grab_set()
        self.ventana.wait_window(self.ventana_bloqueo)
