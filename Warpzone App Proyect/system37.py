...
# Descargar la imagen del ícono y guardarla localmente
url_icono = "https://i.ibb.co/zX34xfD/logo.png"
icono_path = "icono_notificacion.ico"

respuesta = requests.get(url_icono)
if respuesta.status_code == 200:
    # Guardar la imagen como archivo .ico
    with open("temp_logo.png", 'wb') as f:
        f.write(respuesta.content)
    
    # Convertir la imagen a formato .ico usando PIL
    img = Image.open("temp_logo.png")
    img.save(icono_path, format='ICO')
    os.remove("temp_logo.png")

def notificaciones_emergentes():
    mensaje1 = (
        "- Recuerda no abrir ni instalar ningún juego de Riot Games.\n"
        "- Recuerda no apagar ni reiniciar la PC."
    )
    mensaje2 = (
        "- Si requieres asistencia, solicita ayuda de soporte.\n"
        "- No instalar software de terceros sin la autorización de un agente de soporte."
    )
    
    while True:
        notification.notify(
            title="Recordatorio de Uso",
            message=mensaje1,
            app_icon=icono_path,
            timeout=15  # Duración de la notificación
        )
        time.sleep(5)  # Esperar 5 segundos antes del siguiente mensaje
        ...
        time.sleep(1200)  # Esperar 5 minutos antes de repetir los mensajes
...
class AplicacionBloqueo:
    def __init__(self):
        ...
        url_fondo = "https://i.ibb.co/GdJYWRH/halo-warpzone.png"
        respuesta = requests.get(url_fondo)
        if respuesta.status_code == 200:
            imagen_fondo = Image.open(BytesIO(respuesta.content))
            self.foto_fondo_pre_cargada = ImageTk.PhotoImage(imagen_fondo)
        else:
            self.foto_fondo_pre_cargada = None
        ...
        if respuesta.status_code == 200:
            ...
            imagen_logo = imagen_logo.resize((150, 150), Image.Resampling.LANCZOS)  # Redimensionar a 150x150
        ...
        self.notificaciones_thread = threading.Thread(target=notificaciones_emergentes)
    ...
    def mostrar_notificacion(self, mensaje):
        notification.notify(
            title="Notificación de Aplicación de Bloqueo",
            message=mensaje,
            app_icon=icono_path,
    ...
    def mostrar_mensaje_grande(self, mensaje):
        self.mostrar_notificacion(mensaje)  # Llamar a la función de notificación
    ...
    def preparar_ventana_bloqueo(self):
        ...
        respuesta = requests.get(url_fondo)
        if respuesta.status_code == 200:
            imagen_fondo = Image.open(io.BytesIO(respuesta.content))
            imagen_fondo = imagen_fondo.resize((pantalla_ancho, pantalla_alto), Image.Resampling.LANCZOS)
            self.foto_fondo = ImageTk.PhotoImage(imagen_fondo)
            label_fondo = tk.Label(self.ventana_bloqueo, image=self.foto_fondo)
        ...
        self.entrada_clave_bloqueo = tk.Entry(contenedor, show="*", font=("Helvetica", 16))
        self.entrada_clave_bloqueo.pack(pady=10)

        boton_desbloquear = tk.Button(contenedor, text="Desbloquear",
                                      command=self.verificar_contrasena_final,
                                      font=("Helvetica", 16))
        boton_desbloquear.pack(pady=10)

        self.ventana_bloqueo.withdraw()  # Inicialmente, ocultar la ventana de bloqueo
    ...
    def crear_ventana_cuenta_regresiva(self):
        self.ventana_cuenta_regresiva = tk.Toplevel(self.ventana)
        ...
        self.ventana_cuenta_regresiva.attributes('-alpha', 0.6)  # Hacer la ventana semi-transparente
        self.ventana_cuenta_regresiva.configure(bg='black')
        self.label_cuenta_regresiva = tk.Label(self.ventana_cuenta_regresiva, text="", font=("Helvetica", 20), bg='black', fg="yellow")
    ...
    def bloquear_pantalla(self):
        self.ventana_bloqueo.deiconify()  # Mostrar la ventana de bloqueo inmediatamente
        self.ventana_bloqueo.update()
        self.ventana.withdraw()  # Ocultar la ventana principal

    def verificar_contrasena_final(self):
        if self.entrada_clave_bloqueo.get() == self.contrasena_guardada:
            self.bloqueo_iniciado = False
