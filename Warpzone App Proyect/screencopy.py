...
class AplicacionBloqueo:
    ...
    def crear_ventana_cuenta_regresiva(self):
        ...
        self.label_cuenta_regresiva = tk.Label(self.ventana_cuenta_regresiva, text="", font=("Helvetica", 10), bg='white', fg="yellow")
    ...
    def temporizador(self, tiempo):
        ...
        while self.bloqueo_iniciado:
            ...
            tiempo_restante_entero = max(int(tiempo_restante), 0)

            # Update the countdown display with whole seconds
            if tiempo_restante_entero <= 10:
                texto = f"{tiempo_restante_entero} segundos"
                self.label_cuenta_regresiva.config(text=texto)
                self.ventana_cuenta_regresiva.update()

            # Check for alerts
            if tiempo_restante_entero < 900 and not alerta_mostrada_15:
                self.mostrar_mensaje_grande("¡15 minutos restantes!")
                alerta_mostrada_15 = True
            elif tiempo_restante_entero < 600 and not alerta_mostrada_10:
                self.mostrar_mensaje_grande("¡10 minutos restantes!")
                alerta_mostrada_10 = True
            elif tiempo_restante_entero < 300 and not alerta_mostrada_5:
